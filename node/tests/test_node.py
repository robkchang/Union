...
def test_tools_text(two_nodes):
    ...
    nowhere = pathlib.Path(a.project_dir).parent / "nowhere"; nowhere.mkdir()
    # An explicit empty project keeps the lazy lookup from walking up from cwd
    # into a real .union (the developer's own node) and rotating its keys.
    assert "not joined" in Tools(None, nowhere).list_agents().lower()
